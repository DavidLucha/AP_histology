...
% Text for loaded data path
ypos = [scrollbar_label_width, 4*scrollbar_height, 1-scrollbar_label_width, scrollbar_height];
gui_data.image_path_text = uicontrol('style','text','units','normalized','position',ypos, ...
...
% Clean tiff images
clean_tiffs(gui_data.image_path, true);
...
if isempty(image_metadata)
    ...
    filter_idx = arrayfun(@(filter) find(cellfun(@(filter_options) ...
        any(strfind(filter,filter_options)),filter_colors(:,1))), ...
        image_metadata.plane_filters);
...
set(gui_data.scrollbar_white, ...
    ...
    'sliderstep',[1,200].*repmat(1/double(max(gui_data.clim(:,2))),1,2));
...
% (matlab bug - scrollbar needs 2x sets to update if last selected)
gui_data.scrollbar_black.Value = gui_data.scrollbar_black.Min;
gui_data.scrollbar_white.Value = gui_data.scrollbar_white.Max;
% (update to actual target value)
